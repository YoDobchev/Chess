...
bool InputHandler::isValidCoordinate(const String& coord) {
...
int InputHandler::charToBoardIndex(char c) {
	if (c >= 'a' && c <= 'h') return c - 'a';
	if (c >= 'A' && c <= 'H') return c - 'A';
	return 0;
}

int InputHandler::charIntToBoardIndex(char c) {
	if (c >= '1' && c <= BOARD_SIZE_CHAR) return c - '1';
	return 0;
}

void InputHandler::token(String& dest, const String& str, int& p) {
	dest.clear();
	while (str[p] == ' ')
		p++;
...
bool InputHandler::areValidParams(String& inputStr, String& error) {
	int p = 0;
	String cmd;
	token(cmd, inputStr, p);
